...
● フォーカス（編集）
アクテイブキーを押しながらウィンドウをクリックするか、
タスクトレイのアイコンをクリックすることで
GhostBoardにフォーカスが与えられ、
選択中の記憶領域を編集できる状態になります。

編集状態に入ってからも
「アクテイブキー＋→/←」でグループを切り替え、
「アクテイブキー＋↑/↓」で記憶領域を切り替えることができます。
...
【HowTo】
● テキストの削除
定型文は永続化されますが履歴テキストは永続化されませんので、
履歴を全て削除したい場合はGhostBoardを再起動すると引き継がれません。
一部のテキストを削除したい場合は上記の「フォーカス（編集）」の操作にて
編集してください。（記憶領域自体を消すことはできません）
