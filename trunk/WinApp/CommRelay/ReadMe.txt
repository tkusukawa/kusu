========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : CommRelay
========================================================================


AppWizard が生成した CommRelay には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには CommRelay を構成している各ファイルの概要説明が含まれています。
...
CommRelay.h
    ...
    みます） をインクルードし、また CCommRelayApp アプリケーションクラスを宣
    言します。

CommRelay.cpp
    このファイルは CCommRelayApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\CommRelay.ico
    ...
    ルです。このアイコンはリソース ファイル CommRelay.rc によってインクルード
    されます。
...
CommRelayDlg.h, CommRelayDlg.cpp - ダイアログ
    これらのファイルは CCommRelayDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な CommRelay.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) CommRelay.pch 
